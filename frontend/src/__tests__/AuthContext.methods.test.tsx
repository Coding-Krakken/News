...
function ActionConsumer() {
  ...
  return (
    <div>
      ...
      <button onClick={() => login({ email: "a@a", password: "p" } as any)}>
        login
      </button>
      <button onClick={() => signup({ email: "b@b", password: "p" } as any)}>
        signup
      </button>
...
describe("AuthContext actions", () => {
  ...
  it("signup sets user", async () => {
    ...
    await waitFor(() =>
      expect(screen.getByText(/user:b@b/)).toBeInTheDocument(),
    );
  });
  ...
  it("logout clears user", async () => {
    ...
    fireEvent.click(screen.getByText("login"));
    await waitFor(() =>
      expect(screen.getByText(/user:x@x/)).toBeInTheDocument(),
    );
