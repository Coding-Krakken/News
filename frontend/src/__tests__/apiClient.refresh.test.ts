...
describe('ApiClient refreshToken', () => {
  ...
  it('refreshes tokens and stores them', () => {
    ...
    mockedAxios.post.mockResolvedValue({ data: { accessToken: 'new-access', refreshToken: 'new-refresh' } });
    ...
    return (apiClient as any).refreshToken().then((token: any) => {
      ...
      expect(mockedAxios.post).toHaveBeenCalledWith(expect.stringContaining('/auth/refresh'), { refreshToken: 'old-refresh' });
    });
  });
