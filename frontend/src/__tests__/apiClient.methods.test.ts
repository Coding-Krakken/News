...
describe("ApiClient methods and token helpers", () => {
  ...
  it("get/post/put/patch/delete delegate to underlying client", async () => {
    ...
    await expect(apiClient.post("/p", { foo: 1 })).resolves.toEqual({
      data: "p",
    });
    await expect(apiClient.put("/u", { foo: 2 })).resolves.toEqual({
      data: "u",
    });
    await expect(apiClient.patch("/pa", { foo: 3 })).resolves.toEqual({
      data: "pa",
    });
  ...
  it("concurrent refreshToken calls reuse the same promise", async () => {
    ...
    mockedAxios.post.mockImplementation(
      () =>
        new Promise((res) =>
          setTimeout(
            () => res({ data: { accessToken: "acc", refreshToken: "ref" } }),
            10,
          ),
        ),
    );
