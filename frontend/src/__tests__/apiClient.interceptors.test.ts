...
describe("ApiClient interceptors", () => {
  ...
  it("response interceptor refreshes token and retries request on 401", async () => {
    ...
    mockedAxios.create = jest.fn(
      () => clientMock as unknown as import("axios").AxiosInstance,
    );
    // axios.post (global) is used for refresh
    mockedAxios.post.mockResolvedValue({
      data: { accessToken: "ref-access", refreshToken: "ref-refresh" },
    });
  ...
  it("response interceptor retries request after refresh and returns retried response", async () => {
    ...
    const callableClient: any = ((req: any) =>
      Promise.resolve({ data: "retried" })) as any;
    callableClient.interceptors = {
      request: { use: jest.fn() },
      response: {
        use: (succ: any, err: any) => {
          responseErrHandler = err;
        },
      },
    };
    ...
    mockedAxios.create = jest.fn(
      () => callableClient as unknown as import("axios").AxiosInstance,
    );
    mockedAxios.post.mockResolvedValue({
      data: { accessToken: "ref-access", refreshToken: "ref-refresh" },
    });
