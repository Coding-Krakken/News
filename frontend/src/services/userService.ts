...
export const userService = {
  ...
  async getPreferences(): Promise<UserPreference> {
    const response = await apiClient.get<{ preferences: UserPreference }>('/users/me/preferences');
    return response.data.preferences;
  },

  async updatePreferences(data: Partial<UserPreference>): Promise<UserPreference> {
    const response = await apiClient.put<{ preferences: UserPreference }>('/users/me/preferences', data);
    return response.data.preferences;
  },
};
