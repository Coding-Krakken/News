...
"""
            ...
            response = client.chat.completions.create(
                ...
                messages=[
                    {
                        "role": "system",
                        "content": "You are a fact extraction assistant. Extract only verifiable factual claims.",
                    },
    ...
    def cross_corroborate(self, claims: List[Claim]) -> List[Claim]:
        ...
        for group in claim_groups:
            if len(group) > 1:
                ...
                base_claim.supporting_sources = list(
                    set([c.attribution for c in group])
                )
    ...
    async def generate_fact_ledger(
        self, story_id: str, articles: List[Article]
    ) -> FactLedger:
        ...
        uncorroborated = [
            c for c in corroborated_claims if not c.is_confirmed and not c.is_disputed
        ]
