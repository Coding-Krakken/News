    ...
    try:
        ...
        for story in stories:
            ...
            await db.stories.update_one(
                {"story_id": story.story_id}, {"$set": story.model_dump()}, upsert=True
            )

            # Update articles with story ID
            for article_url in story.article_ids:
                await db.articles.update_one(
                    {"url": article_url}, {"$set": {"story_id": story.story_id}}
                )
...
async def get_story_coverage(story_id: str, db=Depends(get_database)):
    ...
    return {
        ...
        "coverage_percentage": (
            len(story["sources_covered"]) / len(all_sources) * 100 if all_sources else 0
        ),
    }
