...
async def get_source(
    source_id: str, current_user: User = Depends(get_current_admin_user)
):
...
async def create_source(
    source_data: NewsSourceCreate, current_user: User = Depends(get_current_admin_user)
):
...
async def update_source(
    ...
    # Update source
    await db.sources.update_one({"name": source_id}, {"$set": update_data})
...
async def delete_source(
    ...
    # Log audit entry
    await _log_audit(db, "delete", "source", source_id, current_user.username, {})
...
async def enable_source(
    ...
    # Log audit entry
    await _log_audit(db, "enable", "source", source_id, current_user.username, {})
...
async def disable_source(
    ...
    # Log audit entry
    await _log_audit(db, "disable", "source", source_id, current_user.username, {})
...
async def flag_source(
    source_id: str, reason: str, current_user: User = Depends(get_current_admin_user)
):
    ...
    # Log audit entry
    await _log_audit(
        db, "flag", "source", source_id, current_user.username, {"reason": reason}
    )
...
async def get_admin_stats(current_user: User = Depends(get_current_admin_user)):
    ...
    return {
        ...
        "content": {"articles": total_articles, "stories": total_stories},
    }
