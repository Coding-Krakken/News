...
class TestFactCheckingService:
    ...
    @pytest.mark.asyncio
    async def test_extract_claims_from_article_with_ai(
        self, service, sample_article_data, mock_openai_response
    ):
        ...
        with patch("openai.OpenAI") as mock_openai:
            ...
            mock_completion.choices[0].message.content = (
                '[{"text": "Test claim", "is_factual": true}]'
            )
    ...
    @pytest.mark.asyncio
    async def test_extract_claims_from_article_ai_fallback(
    ...
    @pytest.mark.asyncio
    async def test_extract_claims_multiple_articles(
        self, service, sample_articles_list
    ):
        """Test extracting claims from multiple articles."""
        articles = [Article(**data) for data in sample_articles_list[:2]]

        with patch.object(
            service, "_extract_claims_from_article", new_callable=AsyncMock
        ) as mock_extract:
            mock_claim = Claim(
                text="Test claim",
                attribution="Source",
    ...
    async def test_extract_claims_handles_errors(self, service, sample_articles_list):
        ...
        with patch.object(
            ...
            mock_extract.side_effect = [[mock_claim], Exception("Error")]
    ...
    async def test_generate_fact_ledger(self, service, sample_articles_list):
        ...
        with patch.object(
            service, "extract_claims", new_callable=AsyncMock
        ) as mock_extract:
            ...
            uncorroborated_claim = Claim(
                text="Uncorroborated claim", attribution="Source B", article_url="url2"
            )
    ...
    @pytest.mark.asyncio
    async def test_generate_fact_ledger_with_disputed(
        self, service, sample_articles_list
    ):
