...
async def test_get_current_user_success(monkeypatch):
    ...
    user_record = {"username": "alice", "email": "a@b.com", "hashed_password": "x", "role": "user", "disabled": False}
...
async def test_get_optional_user_none_and_invalid(monkeypatch):
    ...
    monkeypatch.setattr(deps, 'get_current_user', lambda token: (_ for _ in ()).throw(HTTPException(status_code=401)))
...
def test_get_current_active_user_raises_for_disabled():
    ...
    with pytest.raises(HTTPException):
        ...
        import asyncio
        asyncio.get_event_loop().run_until_complete(deps.get_current_active_user(current_user=DummyUser()))
...
def test_get_current_admin_user_forbidden():
    ...
    with pytest.raises(HTTPException):
        import asyncio
        asyncio.get_event_loop().run_until_complete(deps.get_current_admin_user(current_user=DummyUser()))
