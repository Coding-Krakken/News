...
class TestCoverageEdgeCases:
    ...
    async def test_add_source_error(self, client: AsyncClient, monkeypatch):
        ...
        resp = await client.post("/api/articles/sources/add", params={
            "name": "ErrSource",
    ...
    async def test_get_facets_empty(self, client: AsyncClient):
        ...
        data = resp.json()
        assert set(data.keys()) == {"sources", "categories", "geographies", "ideologies"}
    ...
    @pytest.mark.asyncio
    async def test_get_article_by_url_success(self, client: AsyncClient, mock_db, sample_articles_list):
    ...
    @pytest.mark.asyncio
    async def test_get_stories_pagination(self, client: AsyncClient, mock_db, sample_story_data):
    ...
    @pytest.mark.asyncio
    async def test_analytics_filter_by_multiple_criteria(self, client: AsyncClient, mock_db, sample_articles_list):
        ...
        for i, article in enumerate(sample_articles_list[:3]):
            ...
            await mock_db.articles.insert_one(article)
        resp = await client.get("/api/analytics/filter?sources=Source0,Source1&categories=Cat0")
        assert resp.status_code == 200
        data = resp.json()
    ...
    @pytest.mark.asyncio
    async def test_get_articles_limit_and_skip(self, client: AsyncClient, mock_db, sample_articles_list):
        # Insert 5 articles
    ...
    async def test_ingest_articles_success(self, client: AsyncClient, monkeypatch):
        ...
        class FakeArticle:
            def __init__(self):
                ...
                self.model_dump = lambda: {"url": self.url, "title": self.title, "content": self.content}
        async def fake_ingest_all_sources():
            return [FakeArticle()]
        monkeypatch.setattr(ingestion_service, "ingest_all_sources", fake_ingest_all_sources)
        resp = await client.post("/api/articles/ingest")
    ...
    async def test_add_source_missing_params(self, client: AsyncClient):
        resp = await client.post("/api/articles/sources/add", params={"name": "OnlyName"})
        assert resp.status_code == 422

    @pytest.mark.asyncio
    async def test_get_story_coverage_zero_sources(self, client: AsyncClient, mock_db, sample_story_data):
    ...
    @pytest.mark.asyncio
    async def test_analytics_filter_by_date(self, client: AsyncClient, mock_db, sample_articles_list):
        ...
        resp = await client.get(f"/api/analytics/filter?start_date={start}&end_date={end}")
        assert resp.status_code == 200
        data = resp.json()
