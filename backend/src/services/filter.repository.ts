...
export class FilterRepository {
  async create(
    userId: number,
    filterData: CreateSavedFilterDto,
  ): Promise<SavedFilter> {
  ...
  async findById(id: number): Promise<SavedFilter | null> {
    const result = await query("SELECT * FROM saved_filters WHERE id = $1", [
      id,
    ]);
  ...
  async update(
    id: number,
    filterData: UpdateSavedFilterDto,
  ...
  async delete(id: number): Promise<boolean> {
    const result = await query("DELETE FROM saved_filters WHERE id = $1", [id]);
