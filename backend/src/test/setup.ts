...
beforeAll(async () => {
  ...
  if (!connected) {
    ...
    console.warn(
      "Test database not available — skipping migrations and using mocked DB.",
    );
    return;
  }
...
afterEach(async () => {
  await db.query(
    "TRUNCATE TABLE refresh_tokens, saved_filters, bookmarks, user_preferences, users RESTART IDENTITY CASCADE",
  );
});
